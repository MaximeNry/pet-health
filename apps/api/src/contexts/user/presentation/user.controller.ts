 ...
 * the filter.
 *
 * No registration endpoint: users are created only through Google login
 * (see the `auth` module). The argon2 local-auth path is kept as a tested
 * capability but is not exposed over HTTP.
 */
@Controller('users')
