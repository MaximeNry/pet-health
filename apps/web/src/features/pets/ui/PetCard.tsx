...
export function PetCard({ pet, index, onEdit }: PetCardProps) {
  ...
  return (
    <div className="pet-card group relative flex flex-col items-center rounded-lg border border-border bg-surface px-[22px] pb-[22px] pt-[26px] text-center">
      <button
        type="button"
        aria-label={t('editPet', { name: pet.name })}
        onClick={() => onEdit(pet)}
        className="absolute right-3 top-3 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full text-fg-3 opacity-0 transition hover:bg-subtle hover:text-fg-1 focus-visible:opacity-100 group-hover:opacity-100"
      >
